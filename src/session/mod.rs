...
impl SessionManager {
    ...
    /// 保存用户输入到会话
    pub fn save_user_input(&self, session_id: &str, input: &str) -> Result<()> {
        let session_dir = self.get_session_dir(session_id);
    ...
    /// 保存API响应到会话
    pub fn save_response_markdown(&self, session_id: &str, markdown: &str) -> Result<()> {
        let session_dir = self.get_session_dir(session_id);
        let response_file = session_dir.join("response.md");
        
        // 写入响应
        fs::write(&response_file, markdown)
            .context("保存API响应失败")?;
    ...
    /// 保存响应图片到会话
    pub fn save_response_image(&self, session_id: &str, original_image_path: &Path) -> Result<PathBuf> {
        let session_dir = self.get_session_dir(session_id);
        ...
        // 复制图片
        fs::copy(original_image_path, &target_path)
            .context("复制响应图片到会话目录失败")?;
    ...
    pub async fn periodic_cleanup(&self, expiry_days: u64) {
        ...
        if let Ok(entries) = fs::read_dir(&self.sessions_dir) {
                                            ...
                                            match self.cleanup_session_images(session_id) {
                                                Ok(count) => {
                                                    if count > 0 {
                                                        cleaned_sessions += 1;
                                                        cleaned_files += count;
                                                        info!("已清理会话 {} 中的 {} 个图片文件", session_id, count);
                                                    }
                                                },
                                                Err(e) => error!("清理会话 {} 图片时出错: {}", session_id, e),
                                            }
                                        }
                                    }
                                }
            ...
            if cleaned_sessions > 0 {
                info!("定期清理完成: 已处理 {} 个会话，删除 {} 个图片文件", cleaned_sessions, cleaned_files);
            }
        }
    }
}
